...
df <- read_excel("CyberSummitQuestionnaire_Data21July20.xls")
...
- Data Collection and Reporting: report a personality profile(s) of cybersecurity professionals attending CyberSummit20

- Analysis & Insights: identify unique personality features (traits) of cybersecurity professionals when compared to the general workforce population

- Conclusions: share ideas and opinions, non-attributional, through the chat module (so I can benefit from your experience and insight)
...
The JPI-R measures were selected due to the range of personality traits measured and inclusion of 7 measurements for "analytical" and "dependable," traits presumed to enhance performance in highly technical work roles.  This set of measures has been useful in vocational counseling.  
...
Median age is `r medianAge` years old;  median years of experience is `r medianExperience` years.
...
df_cert3 %>% filter (Certifications != "None", n >.03) %>% ggplot() + geom_col(aes(x=reorder(Certifications, n), y=n), fill="skyblue") +
  geom_text(aes( x=Certifications, y=n, label = round(n, 2), ),color="black", vjust= -.1, hjust = .3, size=3.5) +
...
```{r ScaledScores, echo=FALSE, message=FALSE}
personality_results2 <- df_scored2 %>% filter(CyberPro=="Yes") %>% 
  ...
  arrange(Dimension, Age) %>%  mutate(Average = movavg(Score, 41, type="s"))

personality_results2$Age <- as.integer(personality_results2$Age)
  
q <- personality_results2 %>% filter(Age >=29) %>%     
...
## Clustering Analysis
sub-groups within the cybersecurity workforce?
...
dfcorrplot <- df_ed2 %>% select(ID, Degrees) %>% left_join(df_scored3)  %>% select(ID, SelfEfficacy, Age, YearsExperience, Degrees, Proficiency, cluster) %>% column_to_rownames("ID") %>% as.data.frame()
...
## Comparison of profiles to cluster1
...
df_comparison <- df_scored %>%  
        dplyr::select(Group, ID, Gender, Age, Anxiety:Traditional_Values ) %>%
       group_by(ID) %>% 
...
p <- df_comparison %>%  filter(ID %in% c("2", "3", "4", "47", "6" ,"33")) %>%  ggplot(aes(x=reorder(Dimension, abs(Score), .fun=max), y=Score, color=ID, group=ID)) + 
  geom_point(size=3) + 
  coord_flip() +
  geom_hline(yintercept = 0, linetype="dashed") +
  scale_color_manual(values=c("skyblue", "green", "green", "skyblue", "green", "skyblue")) + 
  xlab("") + ylab("z score") + labs(caption = "Note: z scores developed from cluster 1 mean and standard deviation values")

# https://www.datanovia.com/en/blog/gganimate-how-to-create-plots-with-beautiful-animation-in-r/
  
anim <- p + transition_states(ID, transition_length = 3, state_length = 1) 
...
- Responsibility: strong obligation to be honest and upright; strong an inflexibly conscience   